...
$res=mysqli_query($link,"select * from recomend order by Id");//传全结果值
        ...
        //那过来的数据
        $sel=$_POST["sel"];
        $res=mysqli_query($link,"select * from recomend where Id like '%$sel%' stuname like '%$sel%' or stusex like '%$sel%' age like '%$sel%' or recomend like '%$sel%'");
    }//搜索框得到的结果值
    while ($row=mysqli_fetch_array($res)){
echo '<tr align="center">';
echo"<td style='text-align:center'>$row[0]楼$row[2]</td><td style='text-align:center'>$row[1]</td>
<td style='text-align:center'>
<input type='submit' value='编辑' class='layui-btn layui-btn-xs' lay-event='edit' name='upsub$row[0]'>
...
//修改功能
        if(!empty($_POST["upsub$row[0]"])){
            echo '<tr align="center">';
            echo "
 <td style='text-align:center'><input type='text' name='upsn' value='$row[2]' class='layui-input' style='width:100%'> </td>
<td style='text-align:center'><input type='text' name='upsw' value='$row[1]' class='layui-input' style='width:100%'> </td>
<td style='text-align:center'><input type='submit' value='确认修改' name='upsubs$row[0]' class='layui-btn layui-btn-xs' lay-event='edit'></td>";
        ...
        if(!empty($_POST["upsubs$row[0]"])){
            ...
            $upsw=$_POST["upsw"];
            //填入数据
            mysqli_query($link,"update recomend set stuname='$upsn',recomend='$upsw' where Id=$row[0]");
        header('location:#');
        exit;
        }
        //先判断是否点击
...
if(confirm("是否删除")==true){
    location.href="recomend-admir-del.php";
}
</script>';
        }
    }
